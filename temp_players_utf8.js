const playersManager = {
    ...
    async deletePlayer(id, e) {
        if (e) e.stopPropagation();
        if (confirm("┬┐Est├ís seguro de eliminar a esta jugadora?") && window.supabaseClient) {
    ...
    renderPlayersList() {
        ...
        if (filtered.length === 0) {
            grid.innerHTML = '<p class="text-muted" style="grid-column: 1/-1; text-align: center; padding: 40px;">No hay jugadoras en esta categor├¡a.</p>';
            return;
        }
        ...
        content.innerHTML = `
            <div style="padding: 20px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px;">
                    <h2 style="color:white; margin:0;">Ficha T├®cnica</h2>
                ...
                <div class="personal-data" style="margin-top:20px; background: rgba(255,255,255,0.03); padding:15px; border-radius:12px;">
                    ...
                    <p><strong>Fecha Nac.:</strong> ${player.birthdate || '-'} (${this.calculateAge(player.birthdate)} a├▒os)</p>
                    <p><strong>Obra Social:</strong> ${player.insurance || '-'}</p>
                    <p><strong>Tel├®fono:</strong> ${player.phone || '-'}</p>
    ...
    exportPDF() {
        ...
        categories.forEach(cat => {
            ...
            doc.text(`Categor├¡a: ${cat}`, 14, startY);
            ...
            doc.autoTable({
                startY: startY + 5,
                head: [['Alias', 'Nombre', 'DNI', 'F. Nac', 'Pos 1', 'Pos 2', 'Estado', 'Tel├®fono', 'Emergencia']],
                body: tableData,
                theme: 'striped',
